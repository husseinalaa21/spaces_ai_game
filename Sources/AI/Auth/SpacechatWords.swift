...
/// The 2,048-word recovery vocabulary Spacechat uses, byte-identical to the
/// list in the Spacechat iOS client (`NativeRecoveryWords`).
///
/// Stored as space-separated chunks joined and split once on first use rather
/// than as a 2,048-element array literal: the literal form is notoriously
/// slow for the Swift type-checker and bloats the parsed source for no gain.
enum SpacechatWords {
    static let all: [String] = blob.split(separator: " ").map(String.init)
    ...
    private static let blob: String = [
        ...
        "rain raise rally ramp ranch ranger range rapid rare rate rather raven raw razor ready real",
